"""
Cian.ru Parser with Playwright - парсер объявлений с обходом блокировок
"""
...
class CianParserPlaywright:
    """Парсер для cian.ru с использованием Playwright для обхода блокировок"""
    ...
    def _get_page_content(self, url: str) -> Optional[str]:
        ...
        try:
            with sync_playwright() as p:
                logger.info(f"Запуск браузера для {url}")
                ...
                # Переходим на страницу (используем 'load' вместо 'networkidle' - быстрее и надежнее)
                logger.info("Загрузка страницы...")
                try:
                    page.goto(url, wait_until='load', timeout=60000)  # Увеличили timeout до 60 сек

                    # Ждем появления контента (любой из этих селекторов)
                    try:
                        page.wait_for_selector('h1, [data-mark="OfferTitle"], script[type="application/ld+json"]', timeout=10000)
                    except:
                    ...
                    logger.warning(f"Timeout при загрузке, но пытаемся получить контент...")
