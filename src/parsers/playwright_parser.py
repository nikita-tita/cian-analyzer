...
class PlaywrightParser(BaseCianParser):
    ...
    def search_similar_in_building(self, target_property: Dict, limit: int = 20) -> List[Dict]:
        ...
        if not residential_complex:
            logger.warning("⚠️ Не указан ЖК, пробуем извлечь из адреса")
            # Пробуем извлечь ЖК из адреса
            import re
            match = re.search(r'ЖК\s+([А-Яа-яёЁ\s\-\d]+?)(?:,|$)', address)
            if match:
                residential_complex = match.group(1).strip()
                logger.info(f"✓ Извлечен ЖК из адреса: {residential_complex}")
            else:
                # ═══════════════════════════════════════════════════════════════════════════
                # ДОРАБОТКА #6: FALLBACK НА ПОИСК ПО АДРЕСУ ДОМА
                # ═══════════════════════════════════════════════════════════════════════════
                logger.warning("⚠️ ЖК не найден в адресе, пробуем поиск по точному адресу дома")

                # Извлекаем улицу и номер дома из адреса
                # Примеры:
                # "Москва, Центральный округ, улица Арбат, 10" → "улица Арбат, 10"
                # "Санкт-Петербург, Невский проспект, 28" → "Невский проспект, 28"

                # Удаляем город и округ
                address_clean = address
                for city in ['Москва', 'Санкт-Петербург', 'СПб']:
                    address_clean = address_clean.replace(city + ',', '').replace(city, '')

                # Удаляем округи/районы (они идут перед улицей)
                address_clean = re.sub(r'[А-Яа-яёЁ]+\s+(округ|район|АО),?\s*', '', address_clean)
                address_clean = address_clean.strip().strip(',').strip()

                # Извлекаем улицу + номер дома (последние 2-3 части адреса)
                # Например: "улица Арбат, 10" или "Невский проспект, 28"
                address_parts = [p.strip() for p in address_clean.split(',') if p.strip()]

                if len(address_parts) >= 2:
                    # Берем последние 2 части: улица и дом
                    street_and_house = ', '.join(address_parts[-2:])
                    logger.info(f"📍 Извлечен адрес дома: '{street_and_house}'")

                    # Делаем текстовый поиск по адресу
                    import urllib.parse
                    encoded_address = urllib.parse.quote(street_and_house)
                    ...
                    search_params = {
                        ...
                        'text': encoded_address,
                    }

                    url = f"{self.base_url}/cat.php?" + '&'.join([f"{k}={v}" for k, v in search_params.items()])
                    logger.info(f"🔗 URL поиска по адресу: {url}")

                    results = self.parse_search_page(url)
                    logger.info(f"✓ Найдено {len(results)} объявлений по адресу '{street_and_house}'")

                    if results:
                        # Фильтруем результаты - оставляем только с тем же адресом
                        filtered_by_address = []

                        # Ключевые слова из адреса (улица + номер дома)
                        street_keywords = set(street_and_house.lower().replace(',', '').split())

                        for result in results:
                            result_address = result.get('address', '').lower()
                            result_keywords = set(result_address.replace(',', '').split())

                            # Если хотя бы 2 ключевых слова совпадают (например, "арбат" + "10")
                            matching = len(street_keywords & result_keywords)
                            if matching >= 2:
                                filtered_by_address.append(result)

                        logger.info(f"✓ После фильтрации по адресу: {len(filtered_by_address)} объявлений")

                        if filtered_by_address:
                            # Успешно нашли аналоги по адресу!
                            return self._validate_and_prepare_results(filtered_by_address, limit, target_property=target_property)

                    logger.warning(f"⚠️ По адресу '{street_and_house}' ничего не найдено")
                else:
                    logger.warning(f"⚠️ Не удалось извлечь адрес дома из: '{address}'")

                # Последний вариант: широкий поиск
                logger.warning("⚠️ Все методы поиска в доме/ЖК провалились, используется широкий поиск по городу")
