...
class PlaywrightParser(BaseCianParser):
    ...
    def search_similar(self, target_property: Dict, limit: int = 20) -> List[Dict]:
        ...
        ПРИОРИТЕТ для новостроек: Сначала пробуем поиск по ЖК, потом городской поиск

        Уровень 0 (НОВОСТРОЙКИ): Поиск в том же ЖК
        ...
        # ═══════════════════════════════════════════════════════════════════════════
        # УРОВЕНЬ 0: Для НОВОСТРОЕК - сначала пробуем поиск по ЖК
        # ═══════════════════════════════════════════════════════════════════════════
        is_new_building = self._is_new_building(target_property)
        residential_complex = target_property.get('residential_complex')
        ...
        if is_new_building and residential_complex:
            ...
            try:
                results_zhk = self.search_similar_in_building(target_property, limit=limit)
                if len(results_zhk) >= 5:  # Минимум 5 аналогов
                    logger.info(f"✅ УРОВЕНЬ 0: Найдено {len(results_zhk)} аналогов в том же ЖК, поиск завершен")
                    logger.info("=" * 80)
                    return results_zhk[:limit]
                elif len(results_zhk) > 0:
                    logger.warning(f"⚠️ УРОВЕНЬ 0: Найдено только {len(results_zhk)} аналогов в ЖК (мало), продолжаем широкий поиск...")
                else:
                    logger.warning(f"⚠️ УРОВЕНЬ 0: В ЖК '{residential_complex}' не найдено аналогов, переходим к широкому поиску")
            except Exception as e:
                logger.error(f"❌ УРОВЕНЬ 0: Ошибка поиска по ЖК: {e}")
            logger.info("")
