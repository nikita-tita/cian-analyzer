...
def calculate_price_confidence(
    ...
    def convert(price_per_sqm: float) -> float:
        return max(0.0, price_per_sqm * area)
    ...
    note_parts = [
        f"Использовано {sample_size} аналогов (медиана {median_ppsm:,.0f} руб/м2).",
    ]
    if bucket_note:
...
def _volatility_bucket(cv: float | None) -> tuple[str, str]:
    """Классифицирует волатильность по коэффициенту вариации."""
    if cv is None:
