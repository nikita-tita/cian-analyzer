...
class Recommendation:
    ...
    action: str
    expected_result: str
    summary: str = ''  # Краткое описание в одно предложение
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            'icon': self.icon,
            'title': self.title,
            'summary': self.summary or self.title,  # Fallback на title
...
class RecommendationEngine:
    ...
    def _check_pricing(self) -> List[Recommendation]:
        ...
        if overpricing > 15:
            ...
            diff_amount = current_price - fair_price

            recs.append(Recommendation(
                priority=self.CRITICAL,
                icon='⚠️',
                title='Сильная переоценка',
                summary=f'Цена завышена на {diff_amount:,.0f} ₽ — рекомендуем снизить для продажи',
            ...
            recs.append(Recommendation(
                ...
                summary=f'Цена выше рынка на {diff_amount:,.0f} ₽ — стоит рассмотреть снижение',
            ...
            recs.append(Recommendation(
                ...
                summary=f'Цена чуть выше рынка на {diff_amount:,.0f} ₽ — будьте готовы к торгу',
            ...
            # ИНФО: Недооценка
            diff_amount = fair_price - current_price
            recs.append(Recommendation(
                priority=self.INFO,
                icon='💰',
                title='Цена ниже рынка',
                summary=f'Можете поднять цену на {diff_amount:,.0f} ₽ без потери скорости продажи',
            ...
            recs.append(Recommendation(
                ...
                icon='✅',
                title='Цена в рынке',
                summary='Цена соответствует рынку — хорошая стартовая позиция',
    ...
    def _check_presentation(self) -> List[Recommendation]:
        ...
        if renders_only or images_count < 10:
            ...
            # Формируем сообщение и summary в зависимости от ситуации
            if renders_only:
                photo_message = 'Если в объявлении только рендеры — рекомендуем добавить реальные фото. Это повышает доверие покупателей.'
                photo_summary = 'Рекомендуем добавить реальные фото вместо рендеров'
            elif images_count == 0:
                photo_message = 'Не удалось определить количество фото. Убедитесь, что в объявлении достаточно качественных снимков (10+).'
                photo_summary = 'Убедитесь, что фотографий достаточно (рекомендуем 15-20)'
            else:
                photo_message = f'В объявлении {images_count} фото. Для максимального эффекта рекомендуется 15-20 качественных снимков.'
                photo_summary = f'В объявлении {images_count} фото — для лучшего результата добавьте ещё'
            ...
            recs.append(Recommendation(
                ...
                message=f'{photo_message} Качественные фото увеличивают просмотры на {views_increase}% и конверсию на {conversion_increase}%.',
                action=f'При необходимости заказать профессиональную фотосессию (~{cost:,.0f} ₽)',
                expected_result=f'Потенциальное ускорение продажи на {time_saved_months:.0f} мес.',
                roi=roi,
        ...
        if not description or len(description) < 200:
            if not description:
                desc_message = 'Не удалось получить описание объявления. Убедитесь, что оно информативное и подробное.'
                desc_summary = 'Проверьте, что описание достаточно подробное'
            else:
                desc_message = f'Описание довольно краткое ({len(description)} символов). Подробное описание повышает доверие покупателей.'
                desc_summary = f'Описание короткое ({len(description)} симв.) — рекомендуем расширить'
            ...
            recs.append(Recommendation(
                ...
                action='Рекомендуется 300-500 символов с акцентом на уникальные особенности',
                expected_result='Увеличение времени просмотра объявления',
                category='presentation'
            ))

        # Видео-обзор для премиум сегмента
        if self.target.get('price', 0) > 30_000_000:
            recs.append(Recommendation(
                priority=self.MEDIUM,
                icon='🎥',
                title='Видео-обзор',
                summary='Для объектов >30 млн видео увеличивает число обращений на 40-60%',
                message='Для премиум сегмента (>30 млн) видео существенно повышает интерес покупателей.',
                action='Если видео нет — рассмотрите съёмку (3-5 минут)',
                expected_result='Потенциальный рост серьёзных обращений на 40-60%',
                category='presentation',
                financial_impact={
                    'Инвестиция': '~30 000 ₽',
                    'Ожидаемый эффект': 'Рост обращений'
                }
            ))

        # Общая рекомендация по презентации (если нет других замечаний)
        if len(recs) == 0:
            recs.append(Recommendation(
                priority=self.INFO,
                icon='✨',
                title='Презентация',
                summary='Убедитесь, что фото качественные и описание информативное',
                message='Качественная презентация — ключ к быстрой продаже. Убедитесь, что фото сделаны при хорошем освещении, а описание информативное.',
                action='При необходимости обновить фото и описание',
                expected_result='Повышение интереса покупателей',
                category='presentation',
                financial_impact={
                    'Затраты': 'минимальные',
                    'Эффект': 'заметный'
    ...
    def _check_strategy(self) -> List[Recommendation]:
        ...
        recs.append(Recommendation(
            priority=self.INFO,
            icon='📊',
            title='Оптимальная стратегия',
            summary=f'Лучший вариант — сценарий «{best_name}» с доходом {best_expected_value:,.0f} ₽',
            message=(
        ...
        if fast_scenario and max_scenario:
            ...
            if fast_expected > max_expected:
                ...
                recs.append(Recommendation(
                    ...
                    summary=f'Не затягивайте — «выжать максимум» обойдётся дороже на {diff:,.0f} ₽',
                    message=(
        ...
        if liquidity_score:
            ...
            if liquidity_score < 0.9:
                recs.append(Recommendation(
                    ...
                    summary=f'Спрос в сегменте пониженный — закладывайте ~{expected_dom or "?"} мес. на продажу',
                    message=(
                ...
                recs.append(Recommendation(
                    ...
                    summary=f'Сегмент в спросе — можно продать быстрее (~{expected_dom or "?"} мес.)',
                    message=(
            ...
            if target_area < median_area and area_impact < 0:
                ...
                if is_premium:
                    recs.append(Recommendation(
                        priority=self.INFO,
                        icon='💎',
                        title='Компактность — плюс',
                        summary=f'В премиум-сегменте площадь {target_area:.0f}м² — преимущество, не недостаток',
                    ...
                    recs.append(Recommendation(
                        priority=self.INFO,
                        icon='📏',
                        title='Площадь меньше среднего',
                        summary=f'Площадь {target_area:.0f}м² меньше медианы {median_area:.0f}м² — учтите в цене',
                        ...
                        expected_result=f'Корректировка {area_impact:.1f}% оправдана',
                        category='pricing'
                    ))
                ...
                if comparables_areas:
                    ...
                    if target_area > max_comparable * 1.2:
                        recs.append(Recommendation(
                            ...
                            summary=f'Площадь {target_area:.0f}м² больше всех аналогов — может затянуть продажу',
