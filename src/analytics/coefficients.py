...
# ИСПРАВЛЕНО: Реалистичное влияние ремонта на цену за м²
# Основано на исследованиях рынка недвижимости:
# - Отделка влияет на 8-15% стоимости, а не на 50-100%!
# - Основная стоимость - это локация, площадь, планировка

REPAIR_LEVEL_COEFFICIENTS = {
    'черновая': 0.90,      # Без отделки (-10% на затраты)
    'эконом': 0.95,        # Простая отделка (-5%)
    'стандартная': 1.00,   # Базовый уровень (эталон)
    'капитальная': 1.05,   # Хороший ремонт (+5%)
    'улучшенная': 1.08,    # Качественная отделка (+8%)
    'премиум': 1.12,       # Премиум материалы (+12%)
    'люкс': 1.15,          # Элитная отделка (+15%)
    'дизайнерская': 1.18,  # Дизайнерский проект (+18%)
}
...
def get_bathrooms_coefficient(bathroom_count: float) -> float:
    ...
    Поддерживает дробные значения (например, медиана = 1.5).
    """
    ...
    if count < 0:
        return BATHROOMS_COEFFICIENTS[0]
