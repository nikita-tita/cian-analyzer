...
class RealEstateAnalyzer:
    ...
    def generate_price_scenarios(self) -> List[PriceScenario]:
        ...
        # 3 сценария: Быстро / Оптимально (рекомендуем) / Максимум
        scenarios_config = [
            {
                'name': 'Быстро',
                'type': 'fast',
                'description': 'Для тех, кому срочно нужна продажа',
                'start_price_pct': 0.97,  # Сразу ниже рынка
                'expected_final_pct': 0.95,  # С учётом торга
                'time_months': 2,
                'base_probability': 90,
            ...
            },
            {
                'name': 'Оптимально',
                'type': 'optimal',
                'description': 'Баланс цены и скорости — рекомендуем',
                'start_price_pct': 1.02,  # Небольшой запас на торг
                'expected_final_pct': 0.98,  # Итоговая близка к рынку
                'time_months': 4,
                'base_probability': 80,
            ...
            },
            {
                'name': 'Максимум',
                'type': 'maximum',
                'description': 'Для терпеливых — максимальная цена',
                'start_price_pct': 1.08,  # Высокая стартовая
                'expected_final_pct': 1.02,  # Выше рынка
                'time_months': 10,
                'base_probability': 40,
