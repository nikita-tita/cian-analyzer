...
def example_simple_search():
    ...
    print("="*80)
    print("🔍 ПРИМЕР 1: Простой мультиисточниковый поиск (ВСЕ 4 ИСТОЧНИКА)")
    print("="*80)
    ...
    # Используем удобную функцию для поиска по ВСЕМ источникам
    print(f"\n🔄 Поиск аналогов во ВСЕХ источниках (ЦИАН, ДомКлик, Авито, Яндекс)...")
    results = search_across_sources(
        target_property=target_property,
        sources=['cian', 'domclick', 'avito', 'yandex'],  # ВСЕ 4 ИСТОЧНИКА!
...
def example_advanced_search():
    ...
    # Конфигурация поиска - ИСПОЛЬЗУЕМ ВСЕ 4 ИСТОЧНИКА
    config = SearchConfig(
        sources=['cian', 'domclick', 'avito', 'yandex'],  # ВСЕ ИСТОЧНИКИ
