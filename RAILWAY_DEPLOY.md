...
**ВАЖНО:** Приложение использует in-memory хранилище и **НЕ требует** Redis или PostgreSQL!

После деплоя Railway автоматически добавит:
- `PORT` - порт для приложения (не изменяйте)

Дополнительные переменные (опционально):
- `FLASK_ENV=production` - режим production
- `PYTHONUNBUFFERED=1` - для логирования в реальном времени

**НЕ добавляйте** переменные для Redis/PostgreSQL - они не используются!
...
1. **In-memory хранилище**: Приложение хранит сессии в памяти процесса (не требует Redis/PostgreSQL)
2. **PlaywrightParser**: Для парсинга используется PlaywrightParser (требует headless браузер)
3. **Gunicorn**: Вместо встроенного Flask сервера используется production-ready gunicorn
4. **1 worker**: Используется только 1 worker для корректной работы in-memory хранилища
5. **Таймауты**: Установлен таймаут 120 секунд для обработки длительных запросов парсинга
6. **Автоперезапуск**: При падении приложения Railway автоматически перезапустит его (до 10 попыток)

**Важно:** НЕ добавляйте Redis или PostgreSQL сервисы в Railway - они не нужны для этой версии приложения!
